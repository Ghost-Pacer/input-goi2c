...
const (
	QuatScale        = float64(1.0 / (1 << 14))
	EulerScale       = float64(1.0 / 16)
	LinearAccelScale = float64(1.0 / 100)
)
...
func (dev *Dev) readScaledValues(startAddr byte, numWords int, scaleFactor float64) ([]float64, error) {
	...
	for i := 0; i < numWords; i++ {
		out[i] = float64(unscaledWords[i]) * scaleFactor
	}
	return out, nil
}

func (dev *Dev) ReadQuat() ([4]float64, error) {
	var dst [4]float64
...
func (dev *Dev) ReadEuler() ([3]float64, error) {
	var dst [3]float64
...
func (dev *Dev) ReadLinearAccel() ([3]float64, error) {
	var dst [3]float64
